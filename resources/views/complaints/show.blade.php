  ...
  <section class="grid gap-4 xl:grid-cols-[1.65fr_1fr]">
    <div class="space-y-4">
      <article class="crm-panel">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-slate-600">{{ $complaint->customer_name }} |
              {{ $complaint->brand?->name ?? 'Tanpa Brand' }}</p>
          </div>
          <div class="flex flex-wrap gap-2">
            <span
              class="{{ $statusTone($complaint->status) }} flex items-center rounded-full px-2.5 py-1 text-xs font-semibold">{{ $complaint->status }}</span>
            <span class="flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">CAPA
              {{ $complaint->capa_status }}</span>
        ...
        <div class="mt-4 grid gap-3 text-sm text-slate-700 sm:grid-cols-2">
          ...
          <p><span class="font-semibold">CAPA Due Date:</span> {{ $complaint->capa_due_date?->format('d M Y') ?: '-' }}
          </p>
        </div>
      ...
      <article class="crm-panel">
        ...
        @if ($canSubmitCapa)
          <form method="POST" action="{{ route('complaints.capa.submit', $complaint) }}"
            class="mt-3 grid gap-3 rounded-2xl border border-slate-200 p-3">
            @csrf
            @method('PATCH')
            ...
            <input type="date" name="capa_due_date" class="crm-input"
              value="{{ $complaint->capa_due_date?->toDateString() }}" required>
        ...
        @if ($canApproveCapa)
          <form method="POST" action="{{ route('complaints.capa.approve', $complaint) }}"
            class="mt-3 grid gap-3 rounded-2xl border border-emerald-200 p-3">
            @csrf
            @method('PATCH')
          ...
          <form method="POST" action="{{ route('complaints.capa.reject', $complaint) }}"
            class="mt-3 grid gap-3 rounded-2xl border border-rose-200 p-3">
            @csrf
            @method('PATCH')
        ...
        @if ($canCloseCapa)
          <form method="POST" action="{{ route('complaints.capa.close', $complaint) }}"
            class="mt-3 grid gap-3 rounded-2xl border border-slate-200 p-3">
            @csrf
            @method('PATCH')
    ...
    <div class="space-y-4">
      ...
      <article class="crm-panel">
        ...
        @if ($canEdit)
          <form method="POST" action="{{ route('complaints.attachments.store', $complaint) }}"
            enctype="multipart/form-data" class="mt-3 grid gap-3">
            @csrf
            <input name="author" class="crm-input" value="{{ auth()->user()?->name }}" placeholder="Nama petugas">
          ...
          @forelse ($complaint->attachments as $attachment)
            <div class="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2 text-sm">
              <div>
                ...
                <p class="text-xs text-slate-500">{{ number_format($attachment->size / 1024, 1) }} KB |
                  {{ $attachment->uploaded_by }}</p>
              </div>
              <a class="text-cyan-700 hover:text-cyan-900"
                href="{{ route('complaints.attachments.download', [$complaint, $attachment]) }}">Download</a>
            </div>
          @empty
        ...
        <div class="mt-3 max-h-64 space-y-2 overflow-auto pr-1">
          @forelse ($auditLogs as $log)
            <div class="rounded-xl border border-slate-200 p-3 text-xs">
              ...
              <p class="mt-1 text-slate-500">{{ $log->created_at?->format('d M Y H:i:s') }} |
